 ...
 * Description: program takes in name of a PNG file in an Equirectangular projection. Based on the command line input the program
 * will either output a Lambert Cylindrical Equal-area projection, based on the given standard latitude, or the Mollweide projection, as another
 * PNG image.
 * run like this:
 * go run projection.go Input.png Output.png // <- uses mollweide only
 * go run projection.go Input.png Output.png Lambert 25 // <- uses lambert with given standard lat
 * go run projection.go Input.png Output.png Lambert // <- uses lambert with default lat 0
 */
